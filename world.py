...
class SimpleWorld_Spot_ATM(object):
    ...
    Model Modes:
    * Default: Geometric Brownian motion (dS = μS dt + σS dW) with stochastic vol/drift
    * Black-Scholes mode: black_scholes = True - Simple GBM, no options
    * Bachelier mode: bachelier = True - Arithmetic Brownian motion (dS = μ dt + σ dW)
    
    Additional options:
    * To turn off stochastic vol use no_stoch_vol = True
    * To turn off mean reversion of the drift set no_stoch_drift = True
    ...
    def __init__(self, config : Config, dtype=dh_dtype ):
        ...
        bachelier_mode = config("bachelier", False, bool, help="Use Bachelier model with arithmetic Brownian motion instead of geometric. Asset follows dS = μ dt + σ dW (normal distribution)")
        ...
        # validate model modes
        _log.verify( not (bs_mode and bachelier_mode), "Cannot enable both black_scholes and bachelier modes simultaneously")
        ...
        # bachelier mode
        if bachelier_mode:
            # For Bachelier, we can keep options but use different pricing
            # Turn off stochastic features for simplicity (can be enhanced later)
            no_sdrift = True
            no_svol   = True
        if no_sdrift:                    
            kappa_m   = 0.   
        ...
        for j in range(1,nSteps+nIvSteps+ttm_steps):
            ...
            if bachelier_mode:
                ...
                # S(t+dt) = S(t) + μ dt + σ dW
                spot[:,j]  = spot[:,j-1] + rdrift[:,j-1] * dt + rvol[:,j-1] * dW_s[:,j-1]
                # Normalize to maintain mean drift
                spot[:,j] += (drift * dt - np.mean(spot[:,j] - spot[:,j-1]))
            else:
                # Geometric Brownian motion (original Black-Scholes): dS = μS dt + σS dW  
                # S(t+dt) = S(t) * exp((μ - σ²/2) dt + σ dW)
        ...
        if strike <= 0.:
            ...
            if bachelier_mode:
                ...
                # Call price: C = (S - K) * Φ(d) + σ√T * φ(d)
                # where d = (S - K) / (σ√T)
                strike_abs = strike * opt_spot  # absolute strike
                vol_sqrt_t = ivol * sqrtTTM     # σ√T
                d_bachelier = (opt_spot - strike_abs) / vol_sqrt_t
                
                # Handle case where vol_sqrt_t is zero
                d_bachelier = np.where(vol_sqrt_t > 1e-10, d_bachelier, 
                                     np.where(opt_spot > strike_abs, 100.0, -100.0))
                
                call_price = ((opt_spot - strike_abs) * norm.cdf(d_bachelier) + 
                             vol_sqrt_t * norm.pdf(d_bachelier))
                call_delta = norm.cdf(d_bachelier)  # Bachelier delta
                call_vega  = sqrtTTM * norm.pdf(d_bachelier)  # Bachelier vega
            else:
                # Black-Scholes option pricing (original)
        ...
        if strike > 0.:
            if bachelier_mode:
                self.inst_names.append( "ATM Call (Bachelier)" )
            else:
                self.inst_names.append( "ATM Call (Black-Scholes)" )
