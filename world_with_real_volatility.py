"""
Deep Hedging Example Worlds with Real Volatility Data
-----------------------------------------------------
Example world for deep hedging using realistic volatility paths from CSV data.

Modified from world.py to use real volatility time series instead of constant volatility.

June 30, 2022
@author: hansbuehler
Modified to use real volatility data
"""
...
class SimpleWorld_Spot_ATM_RealVol(object):
    """
    Simple World with one asset and one floating ATM option using realistic volatility paths.
    This version loads volatility time series from a CSV file and uses them in simulation.
    
    The asset has stochastic volatility from real data, and a mean-reverting drift.
    Each volatility path in the CSV generates a specified number of samples.
    ...
    * To turn off mean reverrsion of the drift set no_stoch_drift = True
    ...
    def __init__(self, config : Config, dtype=dh_dtype ):
        ...
        config : Config
            Long list. Use the report feature of 'config' for full feature set
                ...
                config  = Config()
                world   = SimpleWorld_Spot_ATM_RealVol(config)
        ...
        # Hard-coded parameters for real volatility (edit these as needed)
        vol_csv_path = "volatility_data.csv"  # Path to CSV file with volatility data
        samples_per_vol_path = 1000           # Number of samples per volatility path
        ...
        # Use actual_nSamples for the rest of the simulation
        ...
        if payoff_f is None:
            # None means zero.
            payoff_f = lambda spots: np.zeros( (spots.shape[0], ) )
        elif isinstance(payoff_f, (int,float)):
            # specify terminal payoff as a fixed number, e.g. 0
            payoff_f = lambda spots: np.full( (spots.shape[0],), float(payoff_f) )
        ...
        # ----------------------------
        _log.info("Loading volatility data from %s", vol_csv_path)
        try:
            vol_data = pd.read_csv(vol_csv_path)
            vol_paths = vol_data.values  # Convert to numpy array
            n_vol_paths, vol_path_length = vol_paths.shape
            _log.info("Loaded %d volatility paths, each with %d time steps", n_vol_paths, vol_path_length)
        except Exception as e:
            _log.throw("Failed to load volatility data from %s: %s", vol_csv_path, str(e))
        
        # Verify volatility data dimensions
        required_vol_steps = nSteps + nIvSteps + ttm_steps
        _log.verify( vol_path_length >= required_vol_steps, 
                    "Volatility paths must have at least %d time steps (nSteps + nIvSteps + ttm_steps = %d + %d + %d), found %d", 
                    required_vol_steps, nSteps, nIvSteps, ttm_steps, vol_path_length )
        
        # Calculate actual total samples (samples_per_vol_path * number of volatility paths)
        # Note: nSamples from config is ignored in favor of samples_per_vol_path * n_vol_paths
        actual_nSamples = samples_per_vol_path * n_vol_paths
        _log.info("Using %d samples per volatility path × %d paths = %d total samples", 
                 samples_per_vol_path, n_vol_paths, actual_nSamples)
        
        # vols - using initial values from first volatility path
        rvol_init = config("rvol", vol_paths[0, 0], float, help="Initial realized volatility", help_default="From first volatility path")
        ivol_init = config("ivol", vol_paths[0, 0], float, help="Initial implied volatility", help_default="Same as realized vol from first path")
        ...
        # simulate with real volatility data
        # ----------------------------------
        # Initialize arrays to collect results from all volatility paths
        all_spots = []
        all_rdrift = []
        all_rvol = []
        all_ivol = []
        ...
        # Loop over each volatility path
        for vol_path_idx in range(n_vol_paths):
            _log.info("Processing volatility path %d/%d", vol_path_idx + 1, n_vol_paths)
            
            # Get the current volatility path (truncate to required length)
            current_vol_path = vol_paths[vol_path_idx, :required_vol_steps]
            
            # Generate random numbers for this volatility path (only need spot and drift)
            dW = np.random.normal(size=(samples_per_vol_path, nSteps+nIvSteps+ttm_steps-1, 2)).astype(self.np_dtype) * sqrtDt
            dW_s = dW[:,:,0]
            dW_m = dW[:,:,0] * rho_ms + math.sqrt(1. - rho_ms**2) * dW[:,:,1]

            # Initialize arrays for this volatility path
            spot = np.zeros((samples_per_vol_path, nSteps+nIvSteps+ttm_steps), dtype=self.np_dtype)
            rdrift = np.full((samples_per_vol_path, nSteps+nIvSteps+ttm_steps), drift, dtype=self.np_dtype)
            rvol = np.zeros((samples_per_vol_path, nSteps+nIvSteps+ttm_steps), dtype=self.np_dtype)
            ivol = np.zeros((samples_per_vol_path, nSteps+nIvSteps+ttm_steps), dtype=self.np_dtype)
            
            # Initialize with volatility from CSV for all samples in this path
            for i in range(samples_per_vol_path):
                rvol[i, :] = current_vol_path  # Use the same volatility path for all samples
                ivol[i, :] = current_vol_path  # Use the same volatility path for all samples
            ...
            # Time evolution for this volatility path
            for j in range(1, nSteps+nIvSteps+ttm_steps):
                # Volatility is already set from CSV data - no evolution needed
            ...
            # Store results for this volatility path
            all_spots.append(spot)
            all_rdrift.append(rdrift)
            all_rvol.append(rvol)
            all_ivol.append(ivol)
        
        # Concatenate results from all volatility paths
        spot = np.concatenate(all_spots, axis=0)
        rdrift = np.concatenate(all_rdrift, axis=0)
        rvol = np.concatenate(all_rvol, axis=0)
        ivol = np.concatenate(all_ivol, axis=0)
        ...
        if strike <= 0.:
            ...
            cost_dC = cost_v * np.abs(call_vega) + cost_s * np.abs(call_delta) + cost_p * abs(call_price)
        ...
        self.unique_id = uniqueHash([config.input_dict(), payoff_f, self.tf_dtype.name, vol_csv_path], parse_functions=True)
        ...
        self.tf_sample_weights = tf.constant(self.sample_weights, dtype=self.tf_dtype)   # must be of size [nSamples,1] https://stackoverflow.com/questions/60399983/how-to-create-and-use-weighted-metrics-in-keras
    ...
    def clone(self, config_overwrite=Config(), **kwargs):
        ...
        ----------
            ...
            **kwargs
                Allows specifying additional overwrites of specific config values, e.g.
                    world.clone( seed=222, samples_per_vol_path=10 )
        ...
        return SimpleWorld_Spot_ATM_RealVol(config)
